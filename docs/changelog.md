...
- **UI-quality PDF export.** New `Download PDF` button generates a
  ...
  The PDF endpoint now forwards `quality_tier` so a "Run deeper
  analysis" → "Download PDF" sequence prints the Tier 1+2 metrics
  the user paid for, not a silent Tier 0 fallback.
...
- **Academic Quality framework.** Replaces the "winner = highest
  ...
  `sentence-transformers`. Tier 1.5 (free, opt-in): BERTScore
  (Zhang et al. 2020). Tier 2 (paid, opt-in): G-Eval pairwise
  ...
  truth exists. The Tier 2 judge cost is audited as a
  `quality_judge` row in `app_events` so it shows up in `/usage`
  aggregates without falsifying the analyze runs' own
  `tokens_json` snapshots. Full walkthrough on the

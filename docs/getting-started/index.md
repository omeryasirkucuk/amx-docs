---
title: Get Started
description: AMX walks your database, reads your documentation and codebase, then drafts a complete description for every table and column, with confidence scores and a human review before anything lands in the live database.
---
...
!!! tip
    AMX supports 12 database backends and 9 LLM providers (including `databricks_serving` as of 0.12.7). The fastest way to evaluate
...
-   [**`/studio`**](../cli/studio.md)

    AMX Studio — same review-and-apply workflow on a local web UI: runs, results, pending queue, profile management.

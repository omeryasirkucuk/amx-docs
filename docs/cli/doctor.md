...
`/doctor` is the diagnostics command. Run it inside an AMX session and it reports
actionable problems with your install, config, and connectivity.

## Why `/doctor` is useful

Connection wizards already test the path they wrote, but profiles drift — keys rotate,
proxies change, drivers get uninstalled with the rest of a venv. `/doctor` is the
fastest way to find the first thing that broke without running a real `/run`.

## Running it

AMX is interactive-only; standalone shell subcommands are disabled. Start the session
and use the slash form:
...
- **`amx` binaries on `PATH`.** Reports every `amx` it finds, with version. This catches
  the version-skew bug class — one `amx` in your global pip install, another in a project
  venv, and confusion about which one ran.
...
typically falls under 0.3 s. A clean run finishes with `✓ /doctor finished`; any ✗ check
prints the remediation inline so you don't have to re-read the table.
...
- **Right after install.** Open `amx`, then `/doctor` confirms you have everything the
  wizard needs.
